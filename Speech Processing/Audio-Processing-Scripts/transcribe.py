...
#   - Then run:
#       python transcribe.py
#   - Only the first N .wav files (sorted by name) will be processed.
#
# Resume behavior:
#   - The script loads existing transcriptions from transcriptions.json.
#   - For each .wav file, if its base name (without .wav extension) is already
#     present as a key in the JSON, that file is skipped.
#   - This allows you to re-run the script multiple times; only new/unseen
#     files are processed.
#
...
#   - Saves progress after each file, so partial results are not lost.
# -----------------------------
...
SR = 16000  # segments are already 16 kHz
...
def main():
    ...
    print(f"Total transcribed: {len(results)}/{total_files}")
