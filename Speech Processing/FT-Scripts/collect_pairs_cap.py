...
def main():
    ...
    for root in SOURCES:
        # Sort for deterministic "first N"
        wav_files = sorted(root.rglob("*.wav"), key=lambda p: str(p))

        for wav in wav_files:
            if copied_pairs >= LIMIT_PAIRS:
                break
    ...
    print("\nDone.")
    print(f"Copied pairs:            {copied_pairs} (limit: {LIMIT_PAIRS})")
